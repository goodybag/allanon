define(function(require){
  ...
  , toJSON: function(options){
      ...
      if (options && options.displayName) {
        obj.displayName = !this.get('loggedIn') ? 'New User?' : ''
          || this.get('screenName')
  ...
  , getCollections: function(options, callback){
      ...
      if (options.withSecondaries){
         if (this.get('collections') && this._addedSecondaries && !options.force)
          return callback ? callback(null, this.get('collections')) : null;
      } else {
        if (this.get('collections') && !options.force)
          return callback ? callback(null, this.get('collections')) : null;
      }
      ...
      api.collections.list(this.get('id'), { limit: 1000 }, function(error, collections){
        ...
        for (var i = 0, l = collections.length; i < l; ++i){
          collections[i].isEditable = !(collections[i].id == 'all' || collections[i].id == 'food');
        }
  ...
  , loginGatedFunction: function(func) {
      var user = this;
      return function() {
        var self = this;
        var args = arguments;
