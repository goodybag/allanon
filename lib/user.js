define(function(require){
  ...
  , addCollection: function(name, callback){
      api.collections.create(this.get('id'), { name: name }, function(error, result){
        if (error) return callback ? callback(error) : troller.error(error);

        this_.get('collections').unshift(result);
        troller.trigger('collections.changed', this_.get('collections'));
