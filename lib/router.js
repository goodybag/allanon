define(function(require){
  ...
  return utils.Router.extend({
    ...
    , 'collections/:id/explore':                          'exploreCollection'
  ...
  , collections: function(){
      ...
      troller.app.changePage('collections', function(error, view){
        ...
        user.getCollections({ withSecondaries: true }, function(error, collections){
          if (error) return troller.error(error);
  ...
  , exploreCollection: function(cid){
      troller.modals.close();
      troller.spinner.spin();

      troller.app.changePage('explore-collection', function(error, view){
        if (error) return troller.error(error);

        api.collections.get(cid, function(error, collection){
          if (error) return troller.error(error);

          view.provideCollection(collection);
          troller.spinner.stop();
          view.render();
        });
      });
