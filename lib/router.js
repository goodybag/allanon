define(function(require){
  ...
  , collections: function(){
      ...
      troller.app.changePage('collections', { resetScroll: true }, function(error, view){
        if (error) return troller.error(error);
  ...
  , exploreCollection: function(cid, pid){
      ...
      api.collections.get(user.get('id'), cid, function(error, collection){
        ...
        troller.app.changePage('explore-collection', { collection: collection, resetScroll: true }, function(error, page){
          page.render();
  ...
  , settings: function(){
      ...
      troller.app.changePage('settings', { resetScroll: true });
    }
  ...
  , myPunchcards: function(){
      ...
      troller.app.changePage('my-punchcards', { renderOnce: true, resetScroll: true });
    }

  , legal: function(){
      troller.modals.close();

      troller.app.changePage('legal', { renderOnce: true, resetScroll: true });
    }

  , privacy: function(){
      troller.modals.close();

      troller.app.changePage('privacy', { renderOnce: true, resetScroll: true });
    }

  , charities: function(){
      troller.modals.close();

      troller.app.changePage('charities', { renderOnce: true, resetScroll: true });
    }

  , 'gb-for-businesses': function(){
      troller.modals.close();

      troller.app.changePage('gb-for-businesses', { renderOnce: true, resetScroll: true });
    }

  , 'about-us': function(){
      troller.modals.close();

      troller.app.changePage('about-us', { renderOnce: true, resetScroll: true });
    }

  , 'how-it-works': function(){
      troller.modals.close();

      troller.app.changePage('how-it-works', { renderOnce: true, resetScroll: true });
    }

  , 'support': function() {
      troller.modals.close();
      troller.app.changePage('support', { renderOnce: true, resetScroll: true});
    }

  , locations: function(){
      troller.modals.close();

      troller.app.changePage('locations', { resetScroll: true });
    }

  , singleLocation: function(id){
      troller.modals.close();

      troller.app.changePage('locations', { businessId: id, resetScroll: true });
    }

  , singleBusiness: function(id, lid){
      troller.modals.close();

      troller.app.changePage('business', { businessId: id, locationId: lid, resetScroll: true }, function(error, view){
        if (error) return troller.error(error);
      });
    }
