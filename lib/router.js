define(function(require){
  ...
  , explore: function(type, productId) {
      if (+type > 0 || +type <= 0) {  // typeof +type === 'number'  && productId == null
        productId = type;
        type = null;
      }
  ...
  , exploreCollection: function(cid, pid){
      ...
      troller.app.changePage('explore-collection', {cid: cid, pid: pid}, function(error, page) {
        // TODO: make this the page's responsibility
        if (pid) troller.modals.open('product-details', { productId: pid }, function(error, modal) {
          modal.once('close', function() {
